...
const shallowDiff = R.curry((a, b) => {
  ...
  R.forEachObjIndexed((v, k) => {
    v !== b[k] && (diffProps += `${k}, `); // eslint-disable-line
  })(a);
...
const shallowEqual = (objA, objB) => {
  ...
  const is = (x, y) =>
    x === y ? x !== 0 || y !== 0 || 1 / x === 1 / y : x !== x && y !== y; // eslint-disable-line
...
const findByProp = R.curry((name, value, arrOrObj) =>
  R.find(
    R.propEq(name, value),
    R.has('length', arrOrObj) ? arrOrObj : R.values(arrOrObj),
  ),
);
const findByPath = R.curry((path, value, arrOrObj) =>
  R.find(
    R.pathEq(path, value),
    R.has('length', arrOrObj) ? arrOrObj : R.values(arrOrObj),
  ),
);
const findIndexBy = R.curry((name, value, arr) =>
  R.findIndex(R.propEq(name, value), arr),
);
const findIndexByPath = R.curry((path, value, arr) =>
  R.findIndex(R.pathEq(path, value), arr),
);
...
const toObjBy = R.curry((name, arr) =>
  R.pipe(
...
const removeBy = R.curry((name, value, arr) =>
  R.reject(R.propEq(name, value))(arr),
);
const removeByPath = R.curry((path, value, arr) =>
  R.reject(R.pathEq(path, value))(arr),
);
